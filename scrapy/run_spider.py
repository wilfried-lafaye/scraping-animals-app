...
def main():
    ...
    if hasattr(
            AnimalsSpider,
            'custom_settings') and 'DOWNLOAD_HANDLERS' in AnimalsSpider.custom_settings:
        cs = dict(AnimalsSpider.custom_settings)
        cs.pop('DOWNLOAD_HANDLERS', None)
        AnimalsSpider.custom_settings = cs

    # Optionally skip MongoDB pipeline when environment variable SKIP_MONGO is
    # set
    if os.environ.get('SKIP_MONGO'):
        settings['ITEM_PIPELINES'] = {}
    else:
        # Ensure MongoDB pipeline is active for direct insertion when running
        # locally
